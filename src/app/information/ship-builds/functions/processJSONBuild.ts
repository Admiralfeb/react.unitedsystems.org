...
export const processJSONBuild = (json: string): { buildName: string, shipName: string, guardian: boolean, powerplay: boolean, engineering: boolean } => {
    ...
    powerplayWeaponNames.forEach(x => {
        if (powerplay === false) {
            const powerplayHardpoint = hardpoints.find(hp => hp?.name?.toLowerCase() === x.toLowerCase());
    ...
    return { buildName, shipName, guardian, powerplay, engineering }
}
