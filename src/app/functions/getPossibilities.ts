...
const getPossibilities = async (systemName: string = 'Arugbal', range: number = 50) => {
    possibilityProgress.next('Retrieving Systems');
    let systemList = await getSystemsinSphere(systemName, range);
    systemList = systemList.filter(v => Object.keys(v.information).length !== 0);

    possibilityProgress.next(`Retrieving Body Information for ${systemList.length} systems`);
    systemList = await getSystemswithRings(systemList);

    possibilityProgress.next(`Retrieving Faction Information for ${systemList.length} systems`);
    systemList = await getSystemswithAnarchyFaction(systemList);

    const possibilities = await finalizePossibilities(systemList);

    console.log({ possibilities });
    possibilityProgress.next(`Process complete: ${possibilities.length} Possible Systems`);
}
...
const finalizePossibilities = async (systemList: ISphereSystem[]) => {
    ...
    for (let i = 0; i < systemList.length; i++) {
        ...
        possibilityProgress.next(`Checking for populated systems. ${i} of ${systemList.length}. System: ${system.name}`)
        ...
        if (closeSystems.length > 0) {
            ...
            possibilities = [...possibilities, { systemName: sysName, distance: dis, missionSystems: closeSys }];
        }
    }
